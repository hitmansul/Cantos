...
type Assessment = {
  ...
  score: number;
  grade: string;
  decision: 'ANALISAR AGORA' | 'AGUARDAR' | 'NÃO ENTRAR' | 'DADOS INSUFICIENTES';
  corners: number | null;
  shots: string | null;
...
function statRow(match: LiveMatch, terms: string[]) {
  return match.liveStats?.find((row) => {
    const text = normalize(`${row.key ?? ''} ${row.label ?? ''}`);
    return terms.some((term) => text.includes(term));
  }) ?? null;
}

function statPair(match: LiveMatch, terms: string[]) {
  const row = statRow(match, terms);
  return row ? `${row.home ?? '—'}–${row.away ?? '—'}` : null;
}

function statTotal(match: LiveMatch, terms: string[]) {
  const row = statRow(match, terms);
  if (!row) return null;
...
function findMovement(match: LiveMatch, movements: Movement[]) {
  const target = normalize(matchLabel(match));
  return movements.find((item) => normalize(item.match) === target) ?? null;
}
...
function buildAssessment(match: LiveMatch, movements: Movement[]): Assessment {
  ...
  const reasons: string[] = [`Partida localizada ao vivo aos ${minute}', placar ${match.homeTeam.score}–${match.awayTeam.score}.`];
  const risks: string[] = [];
...
export default function AiAssistantPage() {
  ...
  function loadRiskSettings() {
    ...
    try {
      ...
      setUsedTodayUnits(stakeToUnits(todayStake, nextUnitValue));
      setUsedMonthUnits(stakeToUnits(monthStake, nextUnitValue));
    } catch {
      setUsedTodayUnits(0);
      setUsedMonthUnits(0);
    }
  ...
  async function refreshSources() {
    setLoading(true);
    try {
      const [liveResponse, movementResponse] = await Promise.all([
  ...
  const answer = useMemo(() => {
    ...
    const named = assessments.find((item) => {
      const home = normalize(item.match.homeTeam.name);
      const away = normalize(item.match.awayTeam.name);
      return (home.length >= 3 && q.includes(home)) || (away.length >= 3 && q.includes(away));
    });
  ...
  const requestedTeamNotFound = useMemo(() => {
    const q = normalize(submitted);
    const asksEvaluation = /(?:vale|devo|posso|compensa|recomenda|entraria)/.test(q);
    if (!asksEvaluation) return false;
    return !answer.length || !answer.some((item) => q.includes(normalize(item.match.homeTeam.name)) || q.includes(normalize(item.match.awayTeam.name)));
  }, [answer, submitted]);
  ...
  const summary = useMemo(() => {
    ...
    const best = answer[0];
    const label = matchLabel(best.match);
  ...
  return (
    <main className="mx-auto min-h-screen w-full max-w-7xl space-y-6 px-4 py-6 sm:px-6 lg:px-8">
      <header className="space-y-2">
        <div className="flex items-center gap-2 text-sm font-bold text-primary"><BrainCircuit className="h-4 w-4" /> IA Conversacional Integrada 2.3</div>
        <h1 className="text-3xl font-black tracking-tight sm:text-4xl">Pergunte à IA Cantos</h1>
        <p className="max-w-3xl text-muted-foreground">Os jogos ao vivo agora são a fonte principal da consulta. A IA não substitui a equipe perguntada por partidas demonstrativas.</p>
      </header>

      <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-6">
        <Metric label="Valor de 1 U" value={money.format(unitValue)} />
        <Metric label="Limite por entrada" value={formatUnits(maxEntryUnits)} />
        <Metric label="Uso diário" value={displayUsage(usedTodayUnits, maxDailyUnits)} />
        <Metric label="Saldo diário" value={formatUnits(remainingDailyUnits)} />
        <Metric label="Uso mensal" value={displayUsage(usedMonthUnits, maxMonthlyUnits)} />
        <Metric label="Jogos ao vivo" value={String(liveMatches.length)} detail={lastAnalysis ? `Atualizado ${new Date(lastAnalysis).toLocaleTimeString('pt-BR')}` : undefined} />
      </section>

      <form onSubmit={submit} className="rounded-3xl border bg-card p-4 shadow-sm sm:p-5">
        <div className="flex flex-col gap-3 sm:flex-row">
          <div className="flex min-h-12 flex-1 items-center gap-3 rounded-2xl border bg-background px-4">
            <MessageSquareText className="h-5 w-5 text-primary" />
            <input value={question} onChange={(event) => setQuestion(event.target.value)} className="min-w-0 flex-1 bg-transparent outline-none" placeholder="Ex.: Vale entrar agora no Calcutta Police?" />
          </div>
          <button disabled={loading} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-2xl bg-primary px-5 font-black text-primary-foreground disabled:opacity-60">
            {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <Search className="h-4 w-4" />}
            {loading ? 'Analisando…' : 'Consultar'}
          </button>
          <button type="button" onClick={() => { loadRiskSettings(); void refreshSources(); }} disabled={loading} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-2xl border px-4 font-bold disabled:opacity-60">
      ...
      <section className="rounded-3xl border bg-card p-5 shadow-sm">
        <div className="mb-4 flex items-center gap-2">
          ...
          <div><h2 className="text-xl font-black">Resposta consultiva</h2><p className="text-sm text-muted-foreground">Consulta: “{submitted}”</p></div>
        </div>
        <div className="mb-4 rounded-2xl border bg-primary/5 p-4 font-semibold leading-6">{summary}</div>
        {answer.length === 0 || requestedTeamNotFound ? (
          <div className="rounded-2xl border border-dashed p-8 text-center text-muted-foreground">Nenhuma partida correspondente foi encontrada nos jogos ao vivo.</div>
        ) : (
          <div className="space-y-3">
            {answer.map((item, index) => (
              <article key={item.match.id} className="rounded-2xl border p-4">
                <div className="text-xs font-black uppercase text-primary">#{index + 1} · {item.decision}</div>
                <h3 className="mt-1 text-lg font-black">{matchLabel(item.match)}</h3>
                <p className="text-sm text-muted-foreground">{item.match.competition || 'Competição não informada'} · {item.match.statusText || 'Ao vivo'} · {item.match.minute}' · {item.match.homeTeam.score}–{item.match.awayTeam.score}</p>
                <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-6">
                  <Mini label="Score ao vivo" value={String(item.score)} />
                  <Mini label="Nota" value={item.grade} />
                  <Mini label="Escanteios" value={item.corners === null ? 'Indisponível' : String(item.corners)} />
                  <Mini label="Chutes" value={item.shots ?? 'Indisponível'} />
                  <Mini label="Ataques perigosos" value={item.dangerousAttacks ?? 'Indisponível'} />
                  <Mini label="Mercado" value={item.movement ? `${item.movement.direction} ${item.movement.relativeChangePct.toFixed(1)}%` : 'Sem movimento'} />
                </div>
                <div className="mt-3 grid gap-3 md:grid-cols-2">
                  <Explanation title="Dados usados" items={item.reasons} />
                  <Explanation title="Limitações e cautela" items={item.risks} />
                </div>
              </article>
            ))}
          </div>
...
function Metric({ label, value, detail }: { label: string; value: string; detail?: string }) {
  return <div className="rounded-2xl border bg-card p-4"><div className="text-xs font-bold uppercase text-muted-foreground">{label}</div><div className="mt-1 text-xl font-black">{value}</div>{detail && <div className="text-xs text-muted-foreground">{detail}</div>}</div>;
}

function Mini({ label, value }: { label: string; value: string }) {
  return <div className="rounded-xl bg-muted px-3 py-2"><div className="text-[10px] font-bold uppercase tracking-wide text-muted-foreground">{label}</div><div className="text-sm font-black">{value}</div></div>;
}

function Explanation({ title, items }: { title: string; items: string[] }) {
  return <div className="rounded-xl bg-muted/40 p-3"><div className="text-xs font-black uppercase text-muted-foreground">{title}</div><ul className="mt-2 space-y-1 text-sm">{items.map((item) => <li key={item}>• {item}</li>)}</ul></div>;
}
