...
type ImportResult = { url: string; ok: boolean; status: number; payload?: any; error?: string; missingMatch?: MissingMatch; skipped?: boolean; source?: 'hub' | 'backfill' | 'statistics' };
type Candidate = { url: string; missingMatch?: MissingMatch; source: ImportResult['source'] };
...
async function alreadyImportedMap() {
  ...
  const rows = await sql`SELECT DISTINCT ON (pdf_key) pdf_key, status, saved_values, metrics_count, imported_at, pdf_url FROM world_cup_pmsr_import_log WHERE competition_key = ${WORLD_CUP_2026_KEY} AND dry_run = false ORDER BY pdf_key, imported_at DESC`;
...
function buildCandidateUrls(match: MissingMatch, minMatchNumber: number, maxMatchNumber: number) {
  ...
  for (let number = minMatchNumber; number <= maxMatchNumber; number += 1) for (const home of homeCodes) for (const away of awayCodes) {
    urls.push(`${PMSR_BASE}/PMSR-M${String(number).padStart(2, '0')}-${home}-V-${away}.pdf`);
    urls.push(`${PMSR_BASE}/PMSR-M${String(number).padStart(2, '0')} ${home} V ${away}.pdf`);
    urls.push(`${PMSR_BASE}/PMSR-M${String(number).padStart(2, '0')}-${away}-V-${home}.pdf`);
    urls.push(`${PMSR_BASE}/PMSR-M${String(number).padStart(2, '0')} ${away} V ${home}.pdf`);
  }
  return unique(urls);
}
...
async function discoverFromPage(url: string, source: 'hub' | 'statistics') {
  const response = await fetch(url, { headers: { accept: 'text/html,*/*' }, cache: 'no-store' });
  if (!response.ok) return { error: `HTTP ${response.status}`, discovered: [] as Candidate[], hints: [] as string[] };
  const html = await response.text();
  const links = extractPmsrLinks(html).map((item) => ({ url: item, source }));
  return { error: null, discovered: links, hints: extractMatchReportHints(html) };
}
async function discoverByBackfill(missing: Awaited<ReturnType<typeof fetchMissingFifa>>, limit: number, minMatchNumber: number, maxMatchNumber: number) {
  ...
  for (const match of missingMatches) {
    for (const url of buildCandidateUrls(match, minMatchNumber, maxMatchNumber)) {
      if (found.length >= limit) return found;
      if (await urlExists(url)) { found.push({ url, missingMatch: match, source: 'backfill' }); break; }
    }
  }
  return found;
}
function readLimit(value: SyncBody['limit'], all: boolean) { if (all || String(value).toLowerCase() === 'all') return 104; return Math.max(1, Math.min(Number(value ?? 3), 104)); }
...
async function runSync(request: NextRequest, body: SyncBody) {
  ...
  if (mode === 'hub' || mode === 'both') {
    const hub = await discoverFromPage(hubUrl, 'hub');
    hubDiscoveredCount = hub.discovered.length; hubError = hub.error; discovered.push(...hub.discovered);
    const stats = await discoverFromPage(statisticsUrl, 'statistics');
    statisticsDiscoveredCount = stats.discovered.length; statisticsHintCount = stats.hints.length; statisticsError = stats.error; discovered.push(...stats.discovered);
  }

  const dedupedDiscovered = uniqueCandidates(discovered);
  const newFromPages = dedupedDiscovered.filter((item) => {
    const previous = importedMap.get(pdfKey(item.url));
    return force || dryRun || !(previous?.status === 'success' && Number(previous.saved_values ?? 0) > 0);
  });
  const selected: Candidate[] = [...newFromPages];
  ...
  const deduped = uniqueCandidates(selected).slice(0, limit);
  const results: ImportResult[] = [];
  for (const item of deduped) {
    const previous = importedMap.get(pdfKey(item.url));
    if (!force && !dryRun && previous?.status === 'success' && Number(previous.saved_values ?? 0) > 0) {
  ...
  return NextResponse.json({
    ...
    behavior: 'incremental: descobre todos os PMSR, remove os já importados e só então aplica o limite do lote.',
    hubUrl,
    statisticsUrl,
    hubError,
    statisticsError,
    hubDiscoveredCount,
    statisticsDiscoveredCount,
    statisticsHintCount,
    alreadyImportedCount: importedMap.size,
    pendingFromPages: newFromPages.length,
    selectedCount: deduped.length,
    importedOk: results.filter((r) => r.ok && !r.skipped).length,
    skipped: results.filter((r) => r.skipped).length,
    failed: results.filter((r) => !r.ok).length,
    totalSavedValues: results.reduce((sum, result) => sum + Number(result.payload?.savedValues ?? 0), 0),
