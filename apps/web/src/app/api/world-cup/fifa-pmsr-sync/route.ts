...
type SyncBody = { dryRun?: boolean; limit?: number | string; hubUrl?: string; statisticsUrl?: string; onlyMissing?: boolean; mode?: 'hub' | 'backfill' | 'both'; maxMatchNumber?: number; minMatchNumber?: number; all?: boolean; force?: boolean };
type MissingMatch = { id?: string | number; fixture_key?: string; home_team_name?: string; away_team_name?: string };
type ImportResult = { url: string; ok: boolean; status: number; payload?: unknown; error?: string; missingMatch?: MissingMatch; skipped?: boolean; source?: 'hub' | 'backfill' | 'statistics' };
...
function absoluteUrl(href: string) {
  ...
  if (href.startsWith('/media/')) return `${FIFA_ORIGIN}${href}`;
  if (href.startsWith('/')) return `${FIFA_COM_ORIGIN}${href}`;
...
function extractMatchReportHints(html: string) {
  const hints: string[] = [];
  const decoded = html.replace(/\\u002F/g, '/').replace(/\\\//g, '/').replace(/&quot;/g, '"');
  const reportRegex = /(https?:\/\/[^\s"'<>]+match-report[^\s"'<>]*|\/[^\s"'<>]+match-report[^\s"'<>]*)/gi;
  let match: RegExpExecArray | null;
  while ((match = reportRegex.exec(decoded))) hints.push(absoluteUrl(match[1]));
  return unique(hints);
}
...
async function alreadyImportedMap() {
  ...
  const rows = await sql`SELECT DISTINCT ON (pdf_key) pdf_key, status, saved_values, metrics_count, imported_at FROM world_cup_pmsr_import_log WHERE competition_key = ${WORLD_CUP_2026_KEY} AND dry_run = false ORDER BY pdf_key, imported_at DESC`;
...
async function fetchMissingFifa(origin: string) { try { const response = await fetch(`${origin}/api/world-cup/fifa-missing-stats`, { cache: 'no-store' }); if (!response.ok) return null; return (await response.json()) as { missingFifa?: MissingMatch[]; missingAllStats?: MissingMatch[] }; } catch { return null; } }
function maybeRelevantToMissing(url: string, missing: Awaited<ReturnType<typeof fetchMissingFifa>>) {
  ...
  return missing.missingFifa.some((item) => { const fixture = normalize(item.fixture_key); const home = normalize(item.home_team_name); const away = normalize(item.away_team_name); return (fixture && fixture.split(' ').some((part) => part.length >= 3 && haystack.includes(part))) || (home && haystack.includes(home)) || (away && haystack.includes(away)); });
}
function codesForTeam(teamName?: string) { return CODE_BY_TEAM[normalize(teamName)] ?? []; }
function buildCandidateUrls(match: MissingMatch, minMatchNumber: number, maxMatchNumber: number) { const homeCodes = codesForTeam(match.home_team_name); const awayCodes = codesForTeam(match.away_team_name); const urls: string[] = []; if (!homeCodes.length || !awayCodes.length) return urls; for (let number = minMatchNumber; number <= maxMatchNumber; number += 1) for (const home of homeCodes) for (const away of awayCodes) { urls.push(`${PMSR_BASE}/PMSR-M${number}-${home}-V-${away}.pdf`); urls.push(`${PMSR_BASE}/PMSR-M${number}-${away}-V-${home}.pdf`); } return unique(urls); }
async function urlExists(url: string) { try { let response = await fetch(url, { method: 'HEAD', cache: 'no-store' }); if (response.ok) return true; if ([403, 405].includes(response.status)) { response = await fetch(url, { method: 'GET', headers: { range: 'bytes=0-64' }, cache: 'no-store' }); return response.ok || response.status === 206; } return false; } catch { return false; } }
...
async function discoverFromPage(url: string, missing: Awaited<ReturnType<typeof fetchMissingFifa>>, limit: number, includeAll: boolean, source: 'hub' | 'statistics') {
  const response = await fetch(url, { headers: { accept: 'text/html,*/*' }, cache: 'no-store' });
  if (!response.ok) return { error: `HTTP ${response.status}`, discovered: [], selected: [], hints: [] };
  const html = await response.text();
  const discovered = extractPmsrLinks(html);
  const hints = extractMatchReportHints(html);
  const pool = includeAll ? discovered : discovered.filter((item) => maybeRelevantToMissing(item, missing));
  return { error: null, discovered, selected: pool.slice(0, limit).map((item) => ({ url: item, source })), hints };
}
async function discoverByBackfill(missing: Awaited<ReturnType<typeof fetchMissingFifa>>, limit: number, minMatchNumber: number, maxMatchNumber: number) { const missingMatches = missing?.missingFifa ?? []; const found: Array<{ url: string; missingMatch: MissingMatch }> = []; for (const match of missingMatches) { const candidates = buildCandidateUrls(match, minMatchNumber, maxMatchNumber); for (const url of candidates) { if (found.length >= limit) return found; if (await urlExists(url)) { found.push({ url, missingMatch: match }); break; } } } return found; }
function readLimit(value: SyncBody['limit'], all: boolean) { if (all || String(value).toLowerCase() === 'all') return 104; return Math.max(1, Math.min(Number(value ?? 20), 104)); }
...
async function runSync(request: NextRequest, body: SyncBody) {
  ...
  let hubError: string | null = null, statisticsError: string | null = null;

  if (mode === 'hub' || mode === 'both') {
    const hub = await discoverFromPage(hubUrl, missing, limit, includeAllHub, 'hub');
    hubDiscoveredCount = hub.discovered.length; hubError = hub.error; selected.push(...hub.selected);
    if (selected.length < limit) {
      const stats = await discoverFromPage(statisticsUrl, missing, limit - selected.length, includeAllHub, 'statistics');
  ...
  for (const item of deduped) {
    ...
    if (previous?.status === 'success' && Number(previous.saved_values ?? 0) > 0) { results.push({ url: item.url, ok: true, status: 200, skipped: true, source: item.source, payload: { skipped: true, reason: 'already_imported', previous } }); continue; }
    const result = await importOne(origin, item.url, dryRun, item.source, item.missingMatch);
    await recordLog(result, dryRun);
    results.push(result);
  }
  return NextResponse.json({ success: true, dryRun, all, force, mode, scope: 'Somente Copa do Mundo 2026. Não altera tabelas ou endpoints de outros campeonatos.', hubUrl, statisticsUrl, hubError, statisticsError, hubDiscoveredCount, statisticsDiscoveredCount, statisticsHintCount, selectedCount: deduped.length, importedOk: results.filter((r) => r.ok && !r.skipped).length, skipped: results.filter((r) => r.skipped).length, failed: results.filter((r) => !r.ok).length, totalSavedValues: results.reduce((sum, result: any) => sum + Number(result.payload?.savedValues ?? 0), 0), searchWindow: { minMatchNumber, maxMatchNumber }, selected: deduped, results, missingSummary: missing ? { missingFifa: missing.missingFifa?.length ?? 0, missingAllStats: missing.missingAllStats?.length ?? 0 } : null, lastUpdated: new Date().toISOString() });
}

export async function GET(request: NextRequest) { const params = request.nextUrl.searchParams; return runSync(request, { dryRun: params.get('dryRun') !== 'false', limit: params.get('limit') ?? undefined, onlyMissing: params.get('onlyMissing') !== 'false', hubUrl: params.get('hubUrl') ?? undefined, statisticsUrl: params.get('statisticsUrl') ?? undefined, mode: (params.get('mode') as SyncBody['mode']) ?? 'both', minMatchNumber: Number(params.get('minMatchNumber') ?? 1), maxMatchNumber: Number(params.get('maxMatchNumber') ?? 104), all: params.get('all') === 'true', force: params.get('force') === 'true' }); }
