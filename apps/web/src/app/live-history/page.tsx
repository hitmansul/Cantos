...
export default function LiveHistoryPage(){
 ...
 useEffect(()=>{let cancelled=false;const loadCatalog=async()=>{try{const response=await fetch(`/api/live/catalog?t=${Date.now()}`,{cache:'no-store'});const data=await response.json() as{matches?:CatalogMatch[];error?:string};if(!response.ok)throw new Error(data.error??'Falha ao carregar jogos ao vivo');if(cancelled)return;const matches=Array.isArray(data.matches)?data.matches:[];setCatalog(matches);setCatalogError(null);const liveIds=new Set(matches.map(match=>match.id));const currentFollowed=readFollowedIds();const kept=currentFollowed.filter(id=>liveIds.has(id));if(kept.length!==currentFollowed.length){writeFollowedIds(kept);setFollowedIds(kept);followRefreshPending=true;setPageGeneration(value=>value+1);}}catch(error){if(!cancelled)setCatalogError(error instanceof Error?error.message:'Falha ao carregar catálogo');}};void loadCatalog();const timer=window.setInterval(()=>void loadCatalog(),45_000);return()=>{cancelled=true;window.clearInterval(timer);};},[]);
