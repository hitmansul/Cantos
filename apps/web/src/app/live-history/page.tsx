...
export default function LiveHistoryPage(){
 ...
 useEffect(()=>{let cancelled=false;const loadCatalog=async()=>{if(document.visibilityState!=='visible')return;try{const response=await fetch(`/api/live/catalog?t=${Date.now()}`,{cache:'no-store'});const data=await response.json() as{matches?:CatalogMatch[];error?:string};if(!response.ok)throw new Error(data.error??'Falha ao carregar jogos ao vivo');if(cancelled)return;const matches=Array.isArray(data.matches)?data.matches:[];setCatalog(matches);setCatalogError(null);}catch(error){if(!cancelled)setCatalogError(error instanceof Error?error.message:'Falha ao carregar catálogo');}};void loadCatalog();const timer=window.setInterval(()=>void loadCatalog(),45_000);const onVisibilityChange=()=>{if(document.visibilityState==='visible')void loadCatalog();};document.addEventListener('visibilitychange',onVisibilityChange);return()=>{cancelled=true;window.clearInterval(timer);document.removeEventListener('visibilitychange',onVisibilityChange);};},[]);
 useEffect(()=>{const originalFetch=window.fetch.bind(window);const wrappedFetch:typeof window.fetch=async(input,init)=>{const effectiveInput=prepareCentralRequest(input);const requestUrl=centralUrl(effectiveInput);const requestedMatchId=requestUrl?.searchParams.get('matchId')??null;const historyMode=requestUrl?.searchParams.get('history')??null;if(requestedMatchId){selectedMatchIdRef.current=requestedMatchId;selectedMissesRef.current=0;}const response=await originalFetch(effectiveInput,init);if(!response.ok||!requestUrl)return response;try{const data=await response.clone().json() as Record<string,unknown>;if(!Array.isArray(data.matches))return response;if(historyMode==='summary'&&selectedMatchIdRef.current){const activeIds=new Set(data.matches.filter((item):item is Record<string,unknown>=>Boolean(item)&&typeof item==='object').map(item=>String(item.id??'')).filter(Boolean));if(activeIds.has(selectedMatchIdRef.current)){selectedMissesRef.current=0;}else{selectedMissesRef.current+=1;if(selectedMissesRef.current>=SELECTED_MISS_LIMIT){historyCache.delete(selectedMatchIdRef.current);selectedMatchIdRef.current=null;selectedMissesRef.current=0;setPageGeneration(value=>value+1);}}}const enriched={...data,matches:data.matches.map(enrichMatch)};const headers=new Headers(response.headers);headers.set('content-type','application/json');headers.delete('content-length');return new Response(JSON.stringify(enriched),{status:response.status,statusText:response.statusText,headers});}catch{return response;}};window.fetch=wrappedFetch;setReady(true);return()=>{if(window.fetch===wrappedFetch)window.fetch=originalFetch;};},[]);
