  ...
  return <main className="mx-auto w-full max-w-7xl space-y-6 px-4 py-6 md:px-8">
    <section className="flex flex-col gap-4 rounded-2xl border border-border bg-card p-5 md:flex-row md:items-center md:justify-between">
      ...
      <div className="flex flex-col items-stretch gap-2 md:items-end">
        <button type="button" onClick={() => void load(true)} disabled={refreshing} className="inline-flex min-w-36 items-center justify-center gap-2 rounded-xl border border-border px-4 py-3 font-semibold hover:bg-muted disabled:cursor-wait disabled:opacity-70">
    ...
    <section className="grid gap-4 lg:grid-cols-[340px_minmax(0,1fr)]">
      <div className="max-h-[72vh] space-y-2 overflow-auto rounded-2xl border border-border bg-card p-3">
        ...
        {matches.map(match => <button key={match.id} onClick={() => setSelectedId(match.id)} className={`w-full rounded-xl border p-3 text-left transition ${match.id === selectedId ? 'border-emerald-500 bg-emerald-500/10' : 'border-border hover:bg-muted/50'}`}>
      ...
      <div className="space-y-4">
        {!selected ? <div className="rounded-2xl border border-dashed border-border bg-card p-10 text-center text-muted-foreground">{initialLoading ? 'Carregando partidas monitoradas...' : 'Nenhuma partida selecionada.'}</div> : <>
          <section className="rounded-2xl border border-border bg-card p-5"><div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between"><div><p className="text-sm text-muted-foreground">{selected.competition}</p><h2 className="mt-1 text-2xl font-black">{selected.homeTeam.name} x {selected.awayTeam.name}</h2><p className="mt-1 text-sm text-muted-foreground">Última leitura: {formatTime(selected.engineUpdatedAt || lastUpdated)}</p></div><div className="rounded-xl bg-emerald-500/10 px-5 py-3 text-center"><p className="text-xs text-muted-foreground">Placar · minuto</p><p className="text-2xl font-black">{selected.homeTeam.score}–{selected.awayTeam.score} · {selected.minute}'</p></div></div></section>
          <section className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><Metric label="Snapshots" value={history.length} /><Metric label="Escanteios atuais" value={selected.corners?.total ?? '—'} /><Metric label="Monitorado desde" value={formatTime(first?.capturedAt)} /><Metric label="Último registro" value={formatTime(latest?.capturedAt)} /></section>
          <section className="rounded-2xl border border-border bg-card p-5"><div className="flex items-center gap-2"><BarChart3 className="h-5 w-5 text-emerald-400" /><h3 className="text-lg font-bold">Ritmo dos últimos 10 minutos</h3></div><div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-5"><div className="rounded-xl border border-border p-3"><p className="text-xs text-muted-foreground">Classificação</p><p className="mt-1 flex items-center gap-2 text-lg font-bold"><TrendIcon value={selected.engineTrend.status} />{labels[selected.engineTrend.status]}</p></div><Metric label="Δ Escanteios" value={signed(selected.engineTrend.cornersDelta)} /><Metric label="Δ Chutes" value={signed(selected.engineTrend.shotsDelta)} /><Metric label="Δ Ataques perigosos" value={signed(selected.engineTrend.dangerousAttacksDelta)} /><Metric label="Δ Tempo parado" value={`${selected.engineTrend.stoppedMinutesDelta.toFixed(1)} min`} /></div></section>
