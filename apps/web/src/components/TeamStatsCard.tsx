...
import { type DetailedTeamStats, calculateAvgLastNGames } from "@/data/teamCornerStats";
...
export function TeamStatsCard({ team, lastNGames = 5 }: TeamStatsCardProps) {
  ...
  const getStats = () => {
    if (timePeriod === "1st") {
      // First half stats - estimate based on proportion
      const firstHalfRatio = team.avgCornersFirstHalf / (team.avgCornersFirstHalf + team.avgCornersSecondHalf);
      return {
        avgFor: team.avgCornersFirstHalf,
        avgHome: +(team.avgCornersHome * firstHalfRatio).toFixed(1),
        avgAway: +(team.avgCornersAway * firstHalfRatio).toFixed(1),
        avgLastN: +(avgLastN * firstHalfRatio).toFixed(1),
        avgWinning: +(team.avgCornersWinning * firstHalfRatio).toFixed(1),
        avgDrawing: +(team.avgCornersDrawing * firstHalfRatio).toFixed(1),
        avgLosing: +(team.avgCornersLosing * firstHalfRatio).toFixed(1),
      };
    } else if (timePeriod === "2nd") {
      // Second half stats
      const secondHalfRatio = team.avgCornersSecondHalf / (team.avgCornersFirstHalf + team.avgCornersSecondHalf);
      return {
        avgFor: team.avgCornersSecondHalf,
        avgHome: +(team.avgCornersHome * secondHalfRatio).toFixed(1),
        avgAway: +(team.avgCornersAway * secondHalfRatio).toFixed(1),
        avgLastN: +(avgLastN * secondHalfRatio).toFixed(1),
        avgWinning: +(team.avgCornersWinning * secondHalfRatio).toFixed(1),
        avgDrawing: +(team.avgCornersDrawing * secondHalfRatio).toFixed(1),
        avgLosing: +(team.avgCornersLosing * secondHalfRatio).toFixed(1),
      };
    }
    // Full game stats
    return {
  ...
  return (
    <Card className="p-5 space-y-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <StatItem 
          label={timePeriod === "full" ? "Média geral" : timePeriod === "1st" ? "Média 1º tempo" : "Média 2º tempo"} 
          value={stats.avgFor}
          icon={CornerUpRight}
          color="text-primary"
      ...
      <div className="space-y-3">
        <h3 className="text-sm font-medium text-muted-foreground flex items-center gap-2">
          ...
          Por situação no jogo {timePeriod !== "full" && <Badge variant="outline" className="text-xs">{timePeriod === "1st" ? "1º Tempo" : "2º Tempo"}</Badge>}
        </h3>
        <div className="grid grid-cols-3 gap-3">
          <div className="p-3 rounded-lg bg-emerald-500/10 border border-emerald-500/20 text-center">
            <TrendingUp className="w-4 h-4 text-emerald-400 mx-auto mb-1" />
            <p className="text-xs text-muted-foreground">Ganhando</p>
            <p className="text-xl font-bold text-emerald-400">{stats.avgWinning}</p>
          </div>
          <div className="p-3 rounded-lg bg-amber-500/10 border border-amber-500/20 text-center">
            <Minus className="w-4 h-4 text-amber-400 mx-auto mb-1" />
            <p className="text-xs text-muted-foreground">Empatando</p>
            <p className="text-xl font-bold text-amber-400">{stats.avgDrawing}</p>
          </div>
          <div className="p-3 rounded-lg bg-red-500/10 border border-red-500/20 text-center">
            ...
            <p className="text-xl font-bold text-red-400">{stats.avgLosing}</p>
          </div>
        </div>
      </div>

      {/* By Half - only show when viewing full game */}
      {timePeriod === "full" && (
        <div className="space-y-3">
