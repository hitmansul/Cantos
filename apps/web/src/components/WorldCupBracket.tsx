...
export function WorldCupBracket(){
  const [feed,setFeed]=useState<Partial<MatchNode>[]>([]); const [loading,setLoading]=useState(false); const [last,setLast]=useState<string|null>(null);
  ...
  return <div className="space-y-4"><div className="flex justify-end"><Button size="sm" variant="outline" onClick={load} disabled={loading}><RefreshCw className={`mr-2 h-4 w-4 ${loading?'animate-spin':''}`}/>Atualizar chave</Button></div><Bracket {...bracket}/><StatsPanel all={bracket.all}/><p className="text-right text-xs text-muted-foreground">Atualizado: {fmtDate(last)}</p></div>;
}
