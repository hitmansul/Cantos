...
function lineMatchesQuery(line: FeaturedLine, query: string) {
  if (!query.trim()) return true;
  const haystack = normalize(
    `${compactMarketLabel(line)} ${line.market} ${line.label} ${line.line} ${line.odds
      .map((odd) => odd.bookmaker)
      .join(' ')}`
  );
...
function FeaturedLineRow({ line, maxOffers = 8 }: { line: FeaturedLine; maxOffers?: number | null }) {
  const best = bestOddForGroup(line.odds);
  const visibleOdds = maxOffers === null ? line.odds : line.odds.slice(0, maxOffers);
  ...
  return (
    <div className="rounded-lg border border-border/60 bg-background/30 p-3">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {maxOffers !== null && line.odds.length > maxOffers && (
          <span className="rounded-full bg-muted px-3 py-1 text-sm text-muted-foreground">
            +{line.odds.length - maxOffers} casas
          </span>
        )}
      </div>
    </div>
...
function AllEventOddsView({
  ...
  const [activeCategory, setActiveCategory] = useState<'corners' | 'cards'>('corners');
  ...
  const activeLines = activeCategory === 'cards' ? cardLines : cornerLines;
  const visibleLines = useMemo(
    () => activeLines.filter((line) => lineMatchesQuery(line, lineQuery)),
    [activeLines, lineQuery]
  );
  const selectedLine = activeLines.find((line) => line.key === selectedLineKey) ?? null;

  useEffect(() => {
    setSelectedLineKey(null);
    setLineQuery('');
  }, [activeCategory, event.id, selectedBookmakers]);
  ...
  return (
    <div className="space-y-4">
      ...
      <Card className="p-4 border-primary/20 bg-muted/20">
        <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <h4 className="flex items-center gap-2 font-semibold">
              <Target className="h-4 w-4 text-emerald-300" />
              Escolha a linha que deseja comparar
            </h4>
            <p className="mt-1 text-sm text-muted-foreground">
              Primeiro selecione o tipo e a linha. Depois mostramos todas as casas para essa linha.
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            <Button
              type="button"
              size="sm"
              variant={activeCategory === 'corners' ? 'default' : 'outline'}
              onClick={() => setActiveCategory('corners')}
              className={activeCategory === 'corners' ? 'bg-emerald-600 hover:bg-emerald-700' : ''}
            >
              Escanteios ({cornerLines.length})
            </Button>
            <Button
              type="button"
              size="sm"
              ...
              className={activeCategory === 'cards' ? 'bg-amber-600 hover:bg-amber-700' : ''}
            >
              Cartões ({cardLines.length})
            </Button>
          </div>
        </div>
        ...
        {!selectedLine && (
          <div className="mb-4">
            <label className="mb-1 block text-sm text-muted-foreground">Buscar linha</label>
            <input
              value={lineQuery}
              onChange={(item) => setLineQuery(item.target.value)}
              placeholder="Ex: 1º tempo 4.5, menos de 9.5, cartões..."
              className="w-full rounded-lg border border-border bg-background px-3 py-2 outline-none focus:border-emerald-500/60"
            />
          </div>
        )}
        ...
        {selectedLine ? (
          <div className="space-y-3">
            <div className="flex items-center justify-between gap-3">
              <p className="text-sm text-muted-foreground">
                Comparando {selectedLine.odds.length} casa(s) para a linha selecionada.
              </p>
              <Button type="button" variant="ghost" size="sm" onClick={() => setSelectedLineKey(null)}>
                Trocar linha
              </Button>
            </div>
            <FeaturedLineRow line={selectedLine} maxOffers={null} />
          </div>
        ) : (
          <div className="grid gap-2 md:grid-cols-2">
            {visibleLines.map((line) => {
              ...
              return (
                <button
                  ...
                  className="rounded-lg border border-border/70 bg-background/40 p-3 text-left transition-colors hover:border-emerald-500/50 hover:bg-emerald-500/10"
                >
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className="font-semibold">{compactMarketLabel(line)}</p>
                      <p className="text-xs text-muted-foreground">{line.label}</p>
                      <p className="mt-1 text-xs text-muted-foreground">{line.odds.length} casa(s)</p>
                    </div>
                    {best && (
                      <Badge className="bg-emerald-500/20 text-emerald-300">
                        Melhor {best.odd.toFixed(2)}
                      </Badge>
                    )}
                  </div>
                </button>
            ...
            {visibleLines.length === 0 && (
              <div className="rounded-lg bg-background/30 p-6 text-center text-sm text-muted-foreground md:col-span-2">
                Nenhuma linha de {activeCategory === 'cards' ? 'cartões' : 'escanteios'} encontrada com esses filtros.
              </div>
            )}
          </div>
        )}
...
function SearchableTeamFilter({
  ...
  return (
    <div className={open ? 'relative z-[1000]' : 'relative z-10'}>
      ...
      {open && (
        <div className="relative z-[1000] mt-2 w-full rounded-lg border border-border bg-popover p-2 shadow-2xl">
...
export function WorldCupOddsAlerts() {
  ...
  const filteredEvents = useMemo(() => {
    ...
    return (data?.events ?? []).filter((event) => {
      ...
      if (lineFilter) {
        const eventLines = groupCornerLines([...event.cornerLines, ...(event.cardLines ?? [])]);
        if (!eventLines.some((line) => lineMatchesQuery(line, lineFilter))) return false;
      }
  ...
  }, [bookmakerFilters, data?.events, lineFilter, teamFilter]);
  ...
  const hasFilters = Boolean(teamFilter || lineFilter || bookmakerFilters.length > 0);
    ...
    <div className="space-y-4">
      ...
      <Card className="relative z-[200] !overflow-visible p-4 border-primary/20">
        ...
        <div className="grid gap-3 lg:grid-cols-3">
          ...
          <div>
            <label className="mb-1 block text-sm text-muted-foreground">Linha</label>
            <input
              value={lineFilter}
              onChange={(item) => setLineFilter(item.target.value)}
              placeholder="Ex: 4.5, 1º tempo, cartões..."
              className="w-full rounded-lg border border-border bg-background px-3 py-2 outline-none focus:border-emerald-500/60"
            />
          </div>
          <div>
