...
function worldCupMatchDedupeKey(match: { startTime: string; homeTeam: string; awayTeam: string }): string {
  ...
  const home = normalizeTeamName(displayWorldCupTeamName(match.homeTeam));
  const away = normalizeTeamName(displayWorldCupTeamName(match.awayTeam));
...
export function WorldCupPage() {
  ...
  const [selectedWorldCupMatch, setSelectedWorldCupMatch] = useState<{
    id: number;
    homeTeam: string;
    awayTeam: string;
    startTime: string;
  ...
  return (
    <div className="space-y-6">
      ...
      <Tabs
        ...
        onValueChange={(value) => {
          setActiveTab(value);
          if (value !== 'agenda') setSelectedWorldCupMatch(null);
        }}
        className="space-y-4"
      >
        ...
        <TabsContent value="agenda" className="space-y-4">
          <Card className="p-4">
            ...
            <WorldCupMatches
              autoLoad
              selectedMatchId={selectedWorldCupMatch?.id ?? null}
              onSelectMatch={(match) => setSelectedWorldCupMatch(match)}
            />
            {selectedWorldCupMatch && (
              <div className="mt-4">
                <FutureMatchPrediction
                  homeTeam={selectedWorldCupMatch.homeTeam}
                  awayTeam={selectedWorldCupMatch.awayTeam}
                  league="Copa do Mundo"
                  kickoff={selectedWorldCupMatch.startTime}
                  kickoffLabel={selectedWorldCupMatch.timeLabel}
                  onClose={() => setSelectedWorldCupMatch(null)}
                />
              </div>
            )}
          </Card>
  ...
  const load = async () => {
    ...
    try {
      ...
      for (const m of rawMatches) {
        ...
        const key = worldCupMatchDedupeKey(normalizedMatch);
        if (!deduped.has(key)) deduped.set(key, normalizedMatch);
      }

      const byDate: Record<string, (typeof matchGroups)[0]> = {};
      const matches = [...deduped.values()].sort((a, b) => {
        ...
        <div key={group.dateLabel} className="space-y-2">
          ...
          <div className="space-y-2">
            {group.matches.map((match) => {
              ...
              const canSelect = !showResults && Boolean(onSelectMatch);
              return (
                <button
                  key={match.id}
                  type="button"
                  ...
                  } ${canSelect ? 'cursor-pointer hover:bg-emerald-500/10 hover:border-emerald-500/30' : 'cursor-default'}`}
                >
