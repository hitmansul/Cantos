...
import { BarChart3, CalendarDays, Flag, RefreshCw, ShieldCheck, Trophy } from 'lucide-react';
...
function stageText(m:AnyMatch){ return norm(`${m.roundName??''} ${m.stage??''} ${m.groupName??''} ${m.league?.round??''}`); }
...
function promoteOfficial(slots: MatchNode[], feedRows: Partial<MatchNode>[], from: string, to: string) { const start=Date.parse(from); const end=Date.parse(to); const candidates = feedRows.filter(m=>{ const t=Date.parse(m.kickoffAt||''); return Number.isFinite(t)&&t>=start&&t<=end&&m.home&&m.away; }).sort((a,b)=>Date.parse(a.kickoffAt||'9999')-Date.parse(b.kickoffAt||'9999')); const used=new Set<number>(); return slots.map((slot,i)=>{ let idx=candidates.findIndex((m,j)=>!used.has(j)&&m.home&&m.away&&unordered(m.home,m.away)===unordered(slot.home,slot.away)); if(idx<0&&candidates[i]&&!used.has(i)) idx=i; if(idx<0) return slot; used.add(idx); return mergeSlot(slot,candidates[idx]); }); }

function Team({ team, score, winner }: { team: string; score?: number | null; winner?: boolean }) {
  return <div className={`flex min-h-[34px] items-center justify-between gap-2 rounded-xl border px-3 py-2 ${winner ? 'border-emerald-400/70 bg-emerald-500/20 text-white' : 'border-white/10 bg-slate-950/75'}`}><span className="truncate">{flag(team)} {display(team)}</span>{typeof score === 'number' && <b>{score}</b>}</div>;
}
function Node({ match }: { match: MatchNode }) {
  const w = winnerOf(match); const pending = !isFinished(match) || !w;
  return <div className={`relative min-w-[230px] rounded-2xl border p-3 shadow-lg ${pending ? 'border-amber-400/35 bg-slate-950/85' : 'border-emerald-400/45 bg-slate-950/90'}`}>
    <div className="mb-2 flex items-center justify-between gap-2"><b className="text-xs uppercase tracking-wide text-emerald-300">{match.stage} {match.slot}</b><span className="rounded-full border border-white/10 bg-white/5 px-2 py-1 text-[10px] text-slate-300">{fmtDate(match.kickoffAt)}</span></div>
    <div className="space-y-2"><Team team={match.home} score={match.homeScore} winner={w !== null && key(w) === key(match.home)} /><Team team={match.away} score={match.awayScore} winner={w !== null && key(w) === key(match.away)} /></div>
    <div className="mt-2 flex flex-wrap gap-1">{typeof match.homeScore==='number'&&typeof match.awayScore==='number'&&<Badge className="bg-emerald-500/20 text-emerald-200 text-[10px]">{match.homeScore} x {match.awayScore}</Badge>}<Badge className={`text-[10px] ${pending?'bg-amber-500/20 text-amber-200':'bg-emerald-500/20 text-emerald-200'}`}>{pending?'pendente':'oficial'}</Badge>{match.venue&&<Badge variant="outline" className="text-[10px]">{match.venue}</Badge>}</div>
  </div>;
}
function Column({ title, matches, className = '' }: { title: string; matches: MatchNode[]; className?: string }) { return <div className={className}><div className="mb-4 rounded-2xl border border-emerald-500/20 bg-emerald-500/10 px-4 py-3 text-sm font-black uppercase tracking-wide text-emerald-300">{title}<span className="ml-2 text-xs font-normal text-slate-400">{matches.length} jogos</span></div><div className="space-y-5">{matches.map(m=><Node key={m.id} match={m}/>)}</div></div>; }
function Bracket({ f32, r16, qf, sf, final, third }: { f32: MatchNode[]; r16: MatchNode[]; qf: MatchNode[]; sf: MatchNode[]; final: MatchNode; third: MatchNode }) {
  return <Card className="overflow-hidden border-emerald-500/20 bg-[radial-gradient(circle_at_center,_rgba(16,185,129,.22),_rgba(2,6,23,.96)_38%,_rgba(2,6,23,1)_72%)]">
    <div className="p-8 text-center"><h2 className="text-4xl font-black text-amber-300 md:text-5xl">CAMINHO ATÉ A FINAL</h2><p className="mt-2 text-sm text-emerald-100">Slots travados: cada vencedor entra apenas no confronto oficial seguinte.</p></div>
    <div className="overflow-x-auto"><div className="grid min-w-[1780px] grid-cols-[1.1fr_1.1fr_1fr_.95fr_.9fr] gap-6 px-7 pb-8">
      <Column title="Fase 32" matches={f32}/><Column title="Oitavas" matches={r16}/><Column title="Quartas" matches={qf}/><Column title="Semifinais" matches={sf}/>
      <div><div className="mb-8 flex justify-center"><div className="rounded-full border border-amber-400/40 bg-amber-500/10 p-8 text-center"><div className="text-6xl">🏆</div><div className="mt-2 font-black tracking-[.35em] text-amber-200">FINAL</div></div></div><Node match={final}/><div className="mt-6"><Node match={third}/></div></div>
    </div></div>
  </Card>;
}
...
export function WorldCupPage(){
  ...
  return <div className="space-y-6"><Card className="border-emerald-500/30 bg-gradient-to-br from-green-950 via-emerald-900 to-teal-950 p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-2xl font-bold text-white">🏆 Copa do Mundo 2026</h2><p className="text-sm text-emerald-200">Mata-mata reconstruído com slots oficiais fixos, sem pular fase e sem final antecipada.</p></div><div className="text-right"><div className="text-xs text-emerald-100">Atualizado: {fmtDate(last)}</div><Button size="sm" variant="outline" onClick={load} disabled={loading}><RefreshCw className={`mr-2 h-4 w-4 ${loading?'animate-spin':''}`}/>Atualizar</Button></div></div></Card>
    <Tabs value={tab} onValueChange={setTab}><TabsList className="grid w-full grid-cols-4"><TabsTrigger value="mata-mata">Mata-mata</TabsTrigger><TabsTrigger value="estatisticas">Estatísticas</TabsTrigger><TabsTrigger value="resultados">Resultados</TabsTrigger><TabsTrigger value="odds">Odds</TabsTrigger></TabsList>
      <TabsContent value="mata-mata"><Bracket {...bracket}/></TabsContent>
      <TabsContent value="estatisticas"><StatsPanel all={bracket.all}/></TabsContent>
      <TabsContent value="resultados" className="space-y-3">{finishedRows.map(m=><ResultCard key={`${m.stage}-${m.slot}`} m={m}/>)}</TabsContent>
