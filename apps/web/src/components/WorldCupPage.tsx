...
const normalizedMatch: WorldCupDisplayMatch = {
id: m.id,
...
if (!previous) {
deduped.set(key, normalizedMatch);
continue;
}

const previousHasScore = previous.homeScore !== undefined || previous.awayScore !== undefined;
const currentHasScore = normalizedMatch.homeScore !== undefined || normalizedMatch.awayScore !== undefined;
const previousHasPortuguese = /[ÁÉÍÓÚÂÊÔÃÕÇáéíóúâêôãõç]/.test(`${previous.homeTeam} ${previous.awayTeam}`);
const currentHasPortuguese = /[ÁÉÍÓÚÂÊÔÃÕÇáéíóúâêôãõç]/.test(`${normalizedMatch.homeTeam} ${normalizedMatch.awayTeam}`);

if ((!previousHasScore && currentHasScore) || (!previousHasPortuguese && currentHasPortuguese)) {
deduped.set(key, normalizedMatch);
}
}
...
byDate[dateLabel].matches.push(m);
}
