...
import { Calendar, Loader2, RefreshCw, Trophy } from 'lucide-react';
...
function fallbackGroups(): StandingGroup[] {
  return GROUP_ORDER.map((key) => ({
    ...
    label: `Grupo ${key}`,
    source: 'estrutura',
    rows: FALLBACK_GROUPS[key].map((teamName, index) => ({
      position: index + 1,
      teamName,
      played: 0,
      won: 0,
...
function normalizeGroups(groups: ApiStandingGroup[]): StandingGroup[] {
  const mapped = groups
    ...
    .map((group, index) => {
      const key = groupKey(group.name || GROUP_ORDER[index]);
      const rows = (group.rows ?? [])
        .map((row, rowIndex) => ({
          position: Number(row.position ?? rowIndex + 1),
...
function qualificationBadge(row: StandingRow, bestThirdKeys: Set<string>) {
  const teamKey = normalize(row.teamName);
  ...
  if (row.position === 3 && bestThirdKeys.has(teamKey)) return <Badge className="bg-emerald-500/20 text-emerald-300 border-emerald-500/30">3º classificado</Badge>;
  if (row.position === 3) return <Badge className="bg-amber-500/20 text-amber-300 border-amber-500/30">Disputa 3º</Badge>;
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-border">
            {group.rows.map((row) => {
              const qualified = row.position <= 2 || bestThirdKeys.has(normalize(row.teamName));
              return (
                <tr key={`${group.key}-${row.teamName}`} className={qualified ? 'border-l-2 border-l-emerald-500' : ''}>
                  ...
                  <td className="px-3 py-3">{qualificationBadge(row, bestThirdKeys)}</td>
                </tr>
              );
            })}
          </tbody>
  ...
  return (
    <Card className="p-4 border-border/70">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <div className="mt-1 text-base font-semibold">
            {flag(match.homeTeamName)} {displayName(match.homeTeamName)} {score} {flag(match.awayTeamName)} {displayName(match.awayTeamName)}
          </div>
      ...
      <div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
        <div className="rounded-lg bg-muted/30 p-3"><div className="text-xs text-muted-foreground">Posse</div><div className="font-semibold">{metricText(match.summary.possession)}</div></div>
        <div className="rounded-lg bg-muted/30 p-3"><div className="text-xs text-muted-foreground">Finalizações</div><div className="font-semibold">{metricText(match.summary.shots)}</div></div>
        <div className="rounded-lg bg-muted/30 p-3"><div className="text-xs text-muted-foreground">Chutes no gol</div><div className="font-semibold">{metricText(match.summary.shotsOnGoal)}</div></div>
        <div className="rounded-lg bg-muted/30 p-3"><div className="text-xs text-muted-foreground">Escanteios</div><div className="font-semibold">{metricText(match.summary.corners)}</div></div>
        <div className="rounded-lg bg-muted/30 p-3"><div className="text-xs text-muted-foreground">Amarelos</div><div className="font-semibold">{metricText(match.summary.yellowCards)}</div></div>
        <div className="rounded-lg bg-muted/30 p-3"><div className="text-xs text-muted-foreground">Vermelhos</div><div className="font-semibold">{metricText(match.summary.redCards)}</div></div>
        <div className="rounded-lg bg-muted/30 p-3"><div className="text-xs text-muted-foreground">Faltas</div><div className="font-semibold">{metricText(match.summary.fouls)}</div></div>
        <div className="rounded-lg bg-muted/30 p-3"><div className="text-xs text-muted-foreground">Impedimentos</div><div className="font-semibold">{metricText(match.summary.offsides)}</div></div>
      </div>
    </Card>
  );
}
...
export function WorldCupPage() {
  ...
  const bestThirdKeys = useMemo(() => new Set(bestThirds.slice(0, 8).map((row) => normalize(row.teamName))), [bestThirds]);
    ...
    <div className="space-y-6">
      <Card className="border-emerald-500/30 bg-gradient-to-br from-green-950 via-emerald-900 to-teal-950 p-5">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-emerald-200">Classificação por grupo, melhores terceiros, mata-mata projetado e estatísticas pós-jogo.</p>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
        <TabsList className="grid w-full grid-cols-5">
          ...
          <TabsTrigger value="terceiros">3º colocados</TabsTrigger>
        ...
        <TabsContent value="grupos" className="space-y-4">
          <Card className="border-emerald-500/20 bg-emerald-500/10 p-3 text-sm text-muted-foreground">
            Regulamento aplicado: os 2 primeiros de cada grupo avançam direto e os 8 melhores terceiros também se classificam para a fase de 32.
          </Card>
        ...
        <TabsContent value="terceiros">
          <Card className="p-4">
            <h3 className="mb-3 font-semibold">Ranking dos terceiros colocados</h3>
            <div className="space-y-2">
              {bestThirds.map((row, index) => (
                <div key={`${row.group}-${row.teamName}`} className="grid gap-2 rounded-lg bg-muted/30 p-3 text-sm md:grid-cols-[80px_1fr_120px_120px] md:items-center">
                  <span className="text-muted-foreground">{index + 1}. Grupo {row.group}</span>
                  <span className="font-medium">{flag(row.teamName)} {displayName(row.teamName)}</span>
                  <span>{row.points} pts • SG {row.goalDiff > 0 ? `+${row.goalDiff}` : row.goalDiff}</span>
                  <Badge className={index < 8 ? 'bg-emerald-500/20 text-emerald-300 border-emerald-500/30' : 'bg-red-500/10 text-red-300 border-red-500/20'}>{index < 8 ? 'Classifica' : 'Fora'}</Badge>
                </div>
              ))}
            </div>
          </Card>
        ...
        <TabsContent value="mata-mata">
          <Card className="p-4">
            <h3 className="mb-2 flex items-center gap-2 font-semibold"><Trophy className="h-4 w-4 text-amber-400" />Mata-mata projetado</h3>
            <p className="mb-4 text-sm text-muted-foreground">Preparado para a fase de 32. Enquanto a fase de grupos estiver em andamento, a lista mostra os classificados projetados pela tabela atual.</p>
            <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
              {qualified.map((team, index) => (
                <div key={`${team}-${index}`} className="rounded-lg border bg-muted/30 px-3 py-2 text-sm">{index + 1}. {flag(team)} {displayName(team)}</div>
              ))}
            </div>
          </Card>
        </TabsContent>
          ...
          <div className="space-y-3">
            ...
            {finishedMatches.length > 0 ? finishedMatches.map((match) => (
              <Card key={match.id} className="p-4">
                ...
                <div className="mt-1 text-base font-semibold">
                  {flag(match.homeTeam)} {displayName(match.homeTeam)} {typeof match.homeScore === 'number' ? match.homeScore : ''} x {typeof match.awayScore === 'number' ? match.awayScore : ''} {flag(match.awayTeam)} {displayName(match.awayTeam)}
                </div>
                <div className="mt-2 rounded-lg bg-muted/30 p-3 text-sm text-muted-foreground">Resultado importado. As estatísticas detalhadas aparecem acima quando o banco persistente recebe os dados pós-jogo.</div>
              </Card>
