...
type BracketMatch = {
  ...
  source?: 'base' | 'oficial' | 'placeholder';
...
// Ordem oficial usada pelo desenho do chaveamento. Corrige Portugal x Espanha e mantém o restante da árvore consistente.
...
function stageOf(m: BracketMatch) { const s = norm(`${m.stageText ?? ''} ${m.label}`); if (s.includes('semi')) return 'sf'; if (s.includes('quarta') || s.includes('quarter')) return 'qf'; if (s.includes('oitava') || s.includes('round of 16')) return 'r16'; if (s.includes('32') || s.includes('round of 32') || s.includes('fase 32')) return 'f32'; if (s.includes('final')) return 'final'; return ''; }
function inWindow(m: BracketMatch, from: string, to: string) { const t = Date.parse(m.kickoffAt || ''); return Number.isFinite(t) && t >= Date.parse(from) && t <= Date.parse(to); }
...
function flatMatches(payload: unknown): AnyMatch[] { if (!payload || typeof payload !== 'object') return []; const obj = payload as Record<string, any>; const pools = [obj.matches, obj.items, obj.results, obj.fixtures, obj.response, obj.data]; return pools.flatMap((v) => Array.isArray(v) ? v : []); }
function uniqueMatches(rows: BracketMatch[]) { const map = new Map<string, BracketMatch>(); for (const m of rows.filter((x) => key(x.home) && key(x.away))) { const k = `${unorderedKey(m.home, m.away)}|${(m.kickoffAt || '').slice(0, 10)}`; const old = map.get(k); if (!old || finishedStatus(m) || stageOf(m)) map.set(k, m); } return [...map.values()]; }
function mergeF32(official: BracketMatch[]) { const used = new Set<number>(); const out = F32_BASE.map((slot) => { const idx = official.findIndex((m, i) => !used.has(i) && unorderedKey(m.home, m.away) === unorderedKey(slot.home, slot.away)); if (idx < 0) return { ...slot }; used.add(idx); const actual = official[idx]; return { ...slot, ...actual, label: slot.label, kickoffAt: actual.kickoffAt || slot.kickoffAt, winnerOverride: winnerOf(actual) ? null : slot.winnerOverride, source: 'oficial' as const }; });
  const extras = official.filter((m, i) => !used.has(i) && (stageOf(m) === 'f32' || inWindow(m, '2026-06-30T00:00:00.000Z', '2026-07-05T03:30:00.000Z'))).sort(byTime);
  for (const extra of extras) {
    const emptyIndex = out.findIndex((slot) => slot.source === 'placeholder' && !finishedStatus(slot));
    if (emptyIndex < 0) break;
    out[emptyIndex] = { ...out[emptyIndex], ...extra, label: out[emptyIndex].label, kickoffAt: extra.kickoffAt || out[emptyIndex].kickoffAt, source: 'oficial' };
  }
  return out;
}
...
function officialForStage(official: BracketMatch[], stage: 'r16' | 'qf' | 'sf' | 'final') { const windows: Record<string, [string, string]> = { r16: ['2026-07-05T12:00:00.000Z', '2026-07-08T03:30:00.000Z'], qf: ['2026-07-09T12:00:00.000Z', '2026-07-11T23:59:00.000Z'], sf: ['2026-07-14T12:00:00.000Z', '2026-07-16T23:59:00.000Z'], final: ['2026-07-18T12:00:00.000Z', '2026-07-20T23:59:00.000Z'] }; const [from, to] = windows[stage]; return official.filter((m) => stageOf(m) === stage || (stageOf(m) === '' && inWindow(m, from, to))).sort(byTime); }
function mergeRound(slots: BracketMatch[], official: BracketMatch[], stage: 'r16' | 'qf' | 'sf' | 'final') { const byStage = officialForStage(official, stage); const used = new Set<number>(); return slots.map((slot, i) => { let idx = byStage.findIndex((m, j) => !used.has(j) && unorderedKey(m.home, m.away) === unorderedKey(slot.home, slot.away)); if (idx < 0) idx = byStage.findIndex((m, j) => !used.has(j) && (key(m.home) === key(slot.home) || key(m.away) === key(slot.away)) && !slot.home.startsWith('Vencedor') && !slot.away.startsWith('Vencedor')); if (idx < 0 && byStage[i] && !used.has(i)) idx = i; if (idx < 0) return slot; used.add(idx); const actual = byStage[idx]; return { ...slot, ...actual, label: slot.label, kickoffAt: actual.kickoffAt || slot.kickoffAt, source: 'oficial' as const }; }); }
...
export function WorldCupPage() {
  ...
  async function load() {
    setLoading(true);
    try {
      const urls = ['/api/365scores/results/copa_do_mundo', '/api/world-cup/persistent-summary', '/api/world-cup/matches', '/api/world-cup/results'];
      const responses = await Promise.allSettled(urls.map((u) => fetch(u, { cache: 'no-store' }).then((r) => r.ok ? r.json() : null)));
      const rows = responses.flatMap((r) => r.status === 'fulfilled' ? flatMatches(r.value) : []).map(toBracket).filter(Boolean) as BracketMatch[];
      setMatches(uniqueMatches(rows));
  ...
  const bracket = useMemo(() => {
    const phase32 = mergeF32(matches);
    ...
    const r16 = mergeRound(r16Base, matches, 'r16');
    const qfBase = makeRound(r16, [[0, 1], [2, 3], [4, 5], [6, 7]], ['Quartas 1', 'Quartas 2', 'Quartas 3', 'Quartas 4'], QF_TIMES);
    const qf = mergeRound(qfBase, matches, 'qf');
    const sfBase = makeRound(qf, [[0, 1], [2, 3]], ['Semifinal 1', 'Semifinal 2'], SF_TIMES);
    const sf = mergeRound(sfBase, matches, 'sf');
    const finalBase = makeRound(sf, [[0, 1]], ['Final'], ['2026-07-19T19:00:00.000Z']);
    const final = mergeRound(finalBase, matches, 'final');
    return { phase32, r16, qf, sf, final };
  }, [matches]);

  const rows = useMemo(() => matches.filter(finishedStatus).sort((a, b) => byTime(b, a)), [matches]);

  return <div className="space-y-6"><Card className="border-emerald-500/30 bg-gradient-to-br from-green-950 via-emerald-900 to-teal-950 p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-2xl font-bold text-white">🏆 Copa do Mundo 2026</h2><p className="text-sm text-emerald-200">Mata-mata com reconstrução automática por resultados encerrados.</p></div><div className="text-right"><div className="text-xs text-emerald-100">Atualizado: {fmtDate(last)}</div><Button size="sm" variant="outline" onClick={load} disabled={loading}><RefreshCw className={`mr-2 h-4 w-4 ${loading ? 'animate-spin' : ''}`}/>Atualizar</Button></div></div></Card><Tabs value={tab} onValueChange={setTab}><TabsList className="grid w-full grid-cols-3"><TabsTrigger value="mata-mata">Mata-mata</TabsTrigger><TabsTrigger value="resultados">Resultados</TabsTrigger><TabsTrigger value="odds">Odds</TabsTrigger></TabsList><TabsContent value="mata-mata"><Bracket {...bracket}/></TabsContent><TabsContent value="resultados" className="space-y-3">{rows.length === 0 && <Card className="p-4 text-sm text-muted-foreground">Nenhum resultado persistido encontrado.</Card>}{rows.map((m) => <Card key={`${m.label}-${m.home}-${m.away}`} className="p-4"><div className="text-xs text-muted-foreground">{fmtDate(m.kickoffAt)} • {m.stageText ?? 'Copa do Mundo'}</div><div className="font-semibold">{flag(m.home)} {name(m.home)} {m.homeScore ?? '—'} x {m.awayScore ?? '—'} {flag(m.away)} {name(m.away)}</div></Card>)}</TabsContent><TabsContent value="odds"><WorldCupOddsAlerts/></TabsContent></Tabs></div>;
}
