...
function formatSingleMetricAnswer(question: string, selected: any, rows: any[], metric: MetricAlias) {
  ...
  const homeRow = bestTeamMetric(metricRows, selected.home_team_name); const awayRow = bestTeamMetric(metricRows, selected.away_team_name); if (!homeRow || !awayRow) return null;
  const homeValue = numberValue(homeRow); const awayValue = numberValue(awayRow); const key = metricKey(homeRow); const totalLine = key.includes('possession') || key.includes('expected_goals') || key.includes('xg') ? '' : `\nTotal: ${formatValue(homeValue + awayValue, key)}.`;
  return `${competitionLabel()} — ${selected.home_team_name} ${selected.home_score ?? '-'} x ${selected.away_score ?? '-'} ${selected.away_team_name}.\n\n${metric.label[0].toUpperCase()}${metric.label.slice(1)}:\n- ${selected.home_team_name}: ${formatValue(homeValue, key)}\n- ${selected.away_team_name}: ${formatValue(awayValue, key)}${totalLine}\n\nFonte: ${sourceLabel(homeRow.source_key)}.`;
}
...
export async function answerWorldCupFromDatabase(question: string): Promise<string | null> {
  const q = normalize(question); if (!isWorldCupIntent(question)) return null; try { assertPersistentDatabaseConfigured(); } catch { return null; }
  if (q.includes('previsao') || q.includes('proximo jogo')) { const rows = await sql`SELECT id, home_team_name, away_team_name, kickoff_at, group_name, round_name, status FROM world_cup_matches WHERE competition_key = ${WORLD_CUP_2026_KEY} AND kickoff_at >= NOW() - INTERVAL '2 hours' AND LOWER(COALESCE(status, '')) NOT IN ('finished', 'fim', 'final', 'ft') ORDER BY kickoff_at ASC NULLS LAST, id ASC LIMIT 20`; if (rows.length > 0) { const teamFiltered = rows.filter((row) => scoreMatch(question, row.home_team_name, row.away_team_name) > 0); const selectedRows = teamFiltered.length ? teamFiltered : rows.slice(0, 5); const lines = selectedRows.map((row) => `- ${row.home_team_name} x ${row.away_team_name} (${row.kickoff_at ? new Intl.DateTimeFormat('pt-BR', { timeZone: 'America/Sao_Paulo', day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' }).format(new Date(row.kickoff_at)) : 'data não informada'})`).join('\n'); return `Próximos jogos da ${competitionLabel()} encontrados no banco:\n\n${lines}\n\nPara ver escanteios, cartões e xG previstos, abra Copa do Mundo > Próximos e clique no jogo.`; } }
  if (q.includes('classificacao') || q.includes('classificação') || q.includes('tabela') || q.includes('grupo')) { const rows = await sql`SELECT s.group_name, s.position, t.name AS team_name, s.played, s.goal_difference, s.points FROM world_cup_standings s JOIN world_cup_teams t ON t.id = s.team_id WHERE s.competition_key = ${WORLD_CUP_2026_KEY} ORDER BY s.group_name NULLS LAST, s.position, s.points DESC, s.goal_difference DESC, t.name LIMIT 80`; if (rows.length === 0) return 'Ainda não existe classificação da Copa do Mundo gravada no banco persistente.'; const grouped = rows.reduce<Record<string, typeof rows>>((acc, row) => { const group = row.group_name || 'Grupo sem nome'; acc[group] = acc[group] ?? []; acc[group].push(row); return acc; }, {}); return Object.entries(grouped).map(([group, items]) => `${group}:\n${items.map((row) => `${row.position}. ${row.team_name} — ${row.points} pts, ${row.played}J, SG ${row.goal_difference}`).join('\n')}`).join('\n\n'); }
  if (q.includes('resultado') || q.includes('placar') || q.includes('venceu') || q.includes('ganhou') || q.includes('quanto terminou') || q.includes('quanto foi') || q.includes('score')) { const selected = await findAskedMatch(question); if (!selected) return null; let winner = 'Empate'; if (selected.home_score !== null && selected.away_score !== null) { if (Number(selected.home_score) > Number(selected.away_score)) winner = selected.home_team_name; else if (Number(selected.away_score) > Number(selected.home_score)) winner = selected.away_team_name; } else winner = 'Resultado ainda não informado'; return [`${competitionLabel()} — resultado: ${selected.home_team_name} ${selected.home_score ?? '-'} x ${selected.away_score ?? '-'} ${selected.away_team_name}`, '', `Vencedor: ${winner}`, `Status: ${selected.status ?? 'não informado'}`].join('\n'); }
  if (isStatQuestion(question)) { const selected = await findAskedMatch(question); if (!selected) return null; const rows = await getMatchStats(Number(selected.id)); if (rows.length === 0) return `Encontrei o jogo ${selected.home_team_name} x ${selected.away_team_name} no banco da ${competitionLabel()}, mas ainda não há estatísticas gravadas para essa partida.`; const requestedMetric = metricFromQuestion(question); if (requestedMetric) return formatSingleMetricAnswer(question, selected, rows, requestedMetric); return formatAllStatsAnswer(selected, rows); }
  return null;
}
